...
class Updater {
    ...
    public function __construct() {
        if ( defined( 'FLEXIFY_DASHBOARD_RECAPTCHA_DEV_MODE' ) ) {
            add_filter( 'https_ssl_verify', '__return_false' );
            add_filter( 'https_local_ssl_verify', '__return_false' );
            add_filter( 'http_request_host_is_external', '__return_true' );
        }

        $this->plugin_slug = FLEXIFY_DASHBOARD_RECAPTCHA_SLUG;
        $this->version = FLEXIFY_DASHBOARD_RECAPTCHA_VERSION;
        $this->cache_key = 'flexify_dashboard_recaptcha_check_updates';
        $this->cache_data_base_key = 'flexify_dashboard_recaptcha_remote_data';
    ...
    public function add_check_updates_link( $actions, $plugin_file ) {
        if ( $plugin_file === $this->plugin_slug . '/' . $this->plugin_slug . '.php' ) {
            $check_updates_link = '<a href="' . esc_url( add_query_arg( 'flexify_dashboard_recaptcha_check_updates', '1' ) ) . '">' . esc_html__( 'Verificar atualizações', 'flexify-dashboard-recaptcha-addon' ) . '</a>';
            $actions['flexify_dashboard_recaptcha_check_updates'] = $check_updates_link;
        }
    ...
    public function check_manual_update_query_arg( $plugins ) {
        if ( isset( $_GET['flexify_dashboard_recaptcha_check_updates'] ) && $_GET['flexify_dashboard_recaptcha_check_updates'] === '1' ) {
            ...
            if ( $remote_data ) {
                ...
                if ( version_compare( $current_version, $latest_version, '<' )) {
                    ...
                    <script type="text/javascript">
                        if ( ! sessionStorage.getItem('reload_flexify_dashboard_recaptcha_update' ) ) {
                            sessionStorage.setItem('reload_flexify_dashboard_recaptcha_update', 'true');
